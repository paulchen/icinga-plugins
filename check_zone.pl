...
foreach $zone (@zones) {
	...
	my $this_ns = 'B.ROOT-SERVERS.NET'; # Got to start somewhere & J is close ;)
	...
	while ((!$authoritative) && ($fails < 3)) { #keep asking til we get an authoritative answer
		...
		if(!$soa_req) {
			$this_ns = $rrset[1]->nsdname;
			$fails++;
		} elsif(!$soa_req->header->aa) {
			if (defined($soa_req)) {
			...
			$authoritative = 1;	
			$soa_req = $res->send($zone, "NS"); # ask for NS
			@rrset = $soa_req->answer;
			foreach $rr (@rrset) {
				&debug($rr->nsdname);
